...
// const setFinalPath = (allPermutations, coordinates) => {
...
const setFinalPath = (allPermutations, coordinates) => {
    ...
    for (let j = 0; j < optimalPath.length - 1; j++) {
        ...
        lineID = linePart1 < linePart2 ? linePart1 + "-" + linePart2 : linePart2 + "-" + linePart1;
        selectedLines.push(lineID);
        console.log("%cTHE LINE ID IS: ", 'color: #0000ff', lineID);
    }
    ...
    // Iterate through selectedLines to add the active-line class to valid elements
    console.log("This is all the selected line with ID: ")
    selectedLines.forEach(element => {
        let someLine = document.getElementById(element);
        ...
        if (someLine !== null) {
            ...
            // someLine.setAttribute("stroke", "blue")
            someLine.removeAttribute("stroke");
            console.log(someLine)
        }
    });

    // setPathFinallyAgain(selectedLines);
};
