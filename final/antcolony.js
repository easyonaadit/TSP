...
function calculateDistancee(permutation, coordinates) {
    let totalDistance = 0;

    for (let i = 0; i < permutation.length - 1; i++) {
        let dist = distanceBetweenCitiees(permutation[i], permutation[i + 1], coordinates);
        totalDistance += dist;
    }
    ...
    let finalDist = startEndDistancee(startCity, endCity);
    totalDistance += finalDist;

    return totalDistance;
}
function startEndDistancee(city1, city2) {
    return Math.sqrt(Math.pow((city1.X - city2.X), 2) + Math.pow((city1.Y - city2.Y), 2));
}

function distanceBetweenCitiees(city1, city2, coordinates) {
...
function displayFinalTours(coordinates) {
    ...
    displayInterval = setInterval(() => {
        if (iindex >= finalTours.length) {
            ...
            setTimeout(() => {
                ...
                console.log("%cTotal Distance of Optimal Path:", "color: green", calculateDistancee(bestTour, coordinates))
